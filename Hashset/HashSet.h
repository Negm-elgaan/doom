...
template <class C> class HashSet
{
	...
	clsHashTable <C, bool>* Set = nullptr;
	public:

		HashSet()
		{
			Set = new clsHashTable <C, bool>();
		}

		void Insert(C Data)
		{
			Set->Insert(Data, true);
		}
		...
		void Print()
		{
			...
			{
				for (std::pair <C, bool>& Tuple : Set->_PtrArray[i]._VTuple)
				{
